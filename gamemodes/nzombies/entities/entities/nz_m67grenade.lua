...
function ENT:GetExplosionTimer()
	return self.explosiveTime
end

function ENT:SetExplosionTimer( time )
	self.explosiveTime = time
	...
	timer.Simple(time, function()
		if IsValid(self) then
			...
			if self.WidowsWine then
				local zombls = ents.FindInSphere(pos, 280)
				...
				util.BlastDamage(self, owner, pos, 370, self.damage)
			end
